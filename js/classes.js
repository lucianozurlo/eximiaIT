...
class eximia_Before_After {
    constructor($obj = null) {
        ...
        this.$el = {
            $wrap: $obj,
            $before: jQuery('<div class="eximia-before-after-img eximia-before-img"/>').appendTo($obj),
            $after: jQuery('<div class="eximia-before-after-img eximia-after-img-wrap"/>').appendTo($obj),
            $divider: jQuery('<div class="eximia-before-after-divider">\
        ...
        this.$el.$after.children('img').wrap('<div class="eximia-after-img"/>');
        this.$el.$after.children('.eximia-after-img').css('background-image', 'url(' + this.$el.$wrap.attr('data-img-after') + ')');
    ...
    layout() {
        this.offset = this.$el.$wrap.offset().left;
        this.size = this.$el.$wrap.width();
        this.$el.$after.children('.eximia-after-img').width(this.$el.$wrap.width()).height(this.$el.$wrap.height());
    }
    reset() {
        this.current = 50;
        this.target = 50;
...
const baw_axis = new eximia_PanAxis(10);
...
class eximia_Menu {
    constructor(_core = null) {
        ...
        this.core.$dom.header.append('<span class="eximia-active-menu-ind"></span>');
        ...
        this.mobile_menu = new eximia_Mobile_Menu(this.$nav);
    }
    layout() {
        const _self = this;
        let active_pos = '-100%';
...
class eximia_Mobile_Menu {
    constructor($obj) {
        ...
        this.$nav = jQuery('<nav class="eximia-mobile-nav"/>').appendTo(this.$body);
        if (this.$el.parents('header').hasClass('is-frosted')) {
            this.$nav.addClass('is-frosted');
        }
        this.$menu = jQuery('<ul class="eximia-mobile-menu"/>').appendTo(this.$nav);
        this.$el.children('ul').children().each(function () {
            jQuery(this).clone().appendTo(_self.$menu);
        });
        this.$overlay = jQuery('<div class="eximia-mobile-menu-overlay"/>').appendTo(this.$body);
        ...
        jQuery('.eximia-mobile-menu-toggler').on('click', function () {
...
class eximia_Infinite_List {
    constructor($obj = null, cfg = {}) {
        ...
        this.$el.on('mouseenter', '.eximia-infinite-list-item', function () {
            ...
            if ('video' === $preview.attr('data-type')) {
                jQuery(this).children('.eximia-infinite-list-preview').children('video')[0].play();
                $preview.children('video')[0].play();
            }
        }).on('mouseleave', '.eximia-infinite-list-item', function () {
            _self.$el.removeClass('is-hovererd');
            let $preview = _self.$bg_wrap.children('[data-id="' + this.getAttribute('data-id') + '"]');
            if ('video' === $preview.attr('data-type')) {
                jQuery(this).children('.eximia-infinite-list-preview').children('video')[0].pause();
    ...
    init() {
        ...
        this.$list = this.$el.children('.eximia-infinite-list');
        this.$bg_wrap = jQuery('<div class="eximia-infinite-list-bgs"/>').appendTo(this.$el);
        this.$scroll = jQuery('<div class="eximia-infinite-scroll"/>').appendTo(this.$el);
        this.$cloned_list = jQuery('<div class="eximia-infinite-scroll-inner"/>').appendTo(this.$scroll);
    ...
    clone_items() {
        ...
        this.$list.children().each(function (i) {
            ...
            if (_self.first_circle) {
                let $preview = $cloned.children('.eximia-infinite-list-preview');
                ...
                if ('video' === type) {
                    ...
                    <div class="eximial-bg-preview-item${(i === 0 ? ' is-active' : '')}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
                } else {
                    $preview.css('background-image', 'url(' + this_src + ')');
                    _self.$bg_wrap.append(`<div class="eximial-bg-preview-item${(i === 0 ? ' is-active' : '')}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`);
                }
            }
        });
...
class eximia_Marquee {
    constructor($obj = null) {
        ...
        this.$inner = this.$marquee.children('.eximia-marquee-inner');
        this.$content = this.$inner.children();
        this.width = this.$inner.find('.eximia-marquee-list').width();
    ...
    init() {
        this.$inner.wrapInner('<div class="eximia-marquee-inner-wrap"/>');
        this.$inner_cont = this.$inner.children('.eximia-marquee-inner-wrap');
...
class eximia_Masked {
    constructor($obj = null, cfg = {}) {
        ...
        this.cfg = Object.assign({
            media_selector: '.eximia-masked-media',
            content_selector: '.eximia-masked-content',
            dbr: 24,
            ibr: 32
        }, cfg);
...
class eximia_LWP {
    constructor($obj = null) {
        ...
        this.$media = jQuery('<div class="eximia-lwp-media" data-appear="fade-right"/>').prependTo(this.$el);
        this.count = this.$list.children('.eximia-lwp-item').length;
        this.active = 0;
        this.prev = 1;

        this.$list.children('.eximia-lwp-item').each(function (i) {
